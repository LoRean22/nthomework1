...
public:
	...
	virtual void setName(std::string nameFigure)
	{
		name = nameFigure;
	}
};
...
public:
	...
	void setName(std::string nameFigure) override
	{
		name = nameFigure;
	}
};
...
int main()
{
	...
	Figure fig(0);
	fig.setName(namefigure);
	fig.getSidesCount();

	Triangle trian(3);
	Figure* par_trian = &trian;
	par_trian->setName(nameTriangle);
	par_trian->getSidesCount();

	Quadrangle quad(4);
	Figure* par_quad = &quad;
	par_quad->setName(nameQuadrangle);
